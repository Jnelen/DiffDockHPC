#Taken from DiffDock https://github.com/gcorso/DiffDock/commits/main, modified by Jochem Nelen (jnelen@ucam.edu)
import copy
import os
import torch
...
for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
    ...
    try:
        ...
        ligandFilePath = data_list[0]["name"][-1].split('____')[-1]
        ligandName = os.path.basename(ligandFilePath).split('.')[0]
        for rank, pos in enumerate(ligand_pos):

            mol_pred = copy.deepcopy(lig)
            if score_model_args.remove_hs: mol_pred = RemoveHs(mol_pred)

            #make outputPath here
            mol_pred.SetProp("_Name", f"{ligandName}_{rank+1}")
            mol_pred.SetProp("ConfidenceScore", f"{confidence[rank]:.2f}")

            write_mol_with_coords(mol_pred, pos, os.path.join(write_dir, 'VS_DD_' + ligandName + f'_rank{rank+1}_confidence{confidence[rank]:.2f}.sdf'), args.remove_hs)
...
if args.delete_cache == True:
	print(f"Removing cache directories at {test_dataset.full_cache_path} and {confidence_test_dataset.full_cache_path}")
	shutil.rmtree(test_dataset.full_cache_path)
	shutil.rmtree(confidence_test_dataset.full_cache_path)
